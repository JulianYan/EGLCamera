...
import com.android.gallery3d.glrenderer.EglCore;
import com.android.gallery3d.glrenderer.ExtTexture;
import com.android.gallery3d.glrenderer.GLES20Canvas;
import com.android.gallery3d.glrenderer.WindowSurface;
...
final class OpenGLRenderer {
        ...
        try {
            mExecutor.execute(
                    () -> {
                        ...
                        if (setWindowSurface(surfaceSize, surface) || setWindowSurface(mNativeContext, surface)) {
    ...
    private SurfaceTexture resetPreviewTexture(@NonNull Size size) {
        ...
        mPreviewTexture = new SurfaceTexture(getGlTextName(mNativeContext));
    ...
    private void renderLatest() {
        ...
        if (mSurfaceSize != null) {
            ...
            boolean success = renderTexture(timestampNs, mMvpTransform, mMvpDirty,
                    mTextureTransform) || renderTexture(mNativeContext, timestampNs, mMvpTransform, mMvpDirty,
                    mTextureTransform);
            mMvpDirty = false;
    ...
    private int getGlTextName(long nativeContext) {
        if (mUseGl) {
            return mExtTexture.getId();
        }
    ...
    private void initGlContext() {
        ...
        EGLSurface surface = mEglCore.createOffscreenSurface(1,1);
        mEglCore.makeCurrent(surface);

        mCanvas = new GLES20Canvas();
        mExtTexture = new ExtTexture(mCanvas, 0x8D65);
    }

    private boolean setWindowSurface(Size size, @Nullable Surface surface) {

        if (!mUseGl) return false;

        mInputWindowSurface = new WindowSurface(mEglCore, surface, true);
        mInputWindowSurface.makeCurrent();

        mCanvas.setSize(size.getWidth(), size.getHeight());
        Log.d(TAG, "ww:" + size.getWidth() + ", h:" + size.getHeight());

        return true;
    }

    private boolean renderTexture(long timestampNs,
                                  @NonNull float[] mvpTransform,
                                  boolean mvpDirty,
                                  @NonNull float[] textureTransform) {
        ...
        mExtTexture.setSize(mPreviewSize.getHeight(), mPreviewSize.getWidth());

        mCanvas.save();

        int x = 0, y = 0;
        int width = mSurfaceSize.getWidth(), height = mSurfaceSize.getHeight();
        int cx = x + width / 2;
        int cy = y + height / 2;
        ...
        mCanvas.drawTexture(mExtTexture, textureTransform,0, 0, mExtTexture.getWidth(), mExtTexture.getHeight());

        mCanvas.restore();

        //Log.d(TAG, "render w:" + mPreviewSize.getWidth() + ", h:" + mPreviewSize.getHeight());

        mInputWindowSurface.setPresentationTime(timestampNs);
        mInputWindowSurface.swapBuffers();

        return true;
    }

    private void closeGlContext() {
